...
As most engineers would know, engineering is not all sunshine and rainbows -- just when you think everything is going great, something, just _something_, has to pop up -- and this is quite literally what I faced. Many, many times. These issues range from minor foolishness to just plain unfortunate turns of events. 

I would like to start off with my 'bugged' ultrasonic sensor. Before I integrated it, I planned to test its functionality first to prevent chaining issues down the line (a phenomena that I will get into later). After learning, I decided to write my own test code based solely off of how it works -- first, start timer and send pulse, receive pulse, use time.time() to find elapsed time, then multiply by speed of sound, etc... My code, unsurprisingly, swifly broke down during runtime. After countless revisions, I went onto a niche forum, that had the audacity to say the obvious: the ultrasonic sensor has its own built-in function and library. Surprise surprise, all this time I had been trying to create something that had already been done since the concept of the sensor itself was born. THis I attribute to inexperience, and perhaps it is fortunate that I learned it the hard way for I would remember it long after. 

Second was my pair of motors. I used a pair of yellow "hobby" DC motors, with two frail and thin mini copper extrusions as ports for current to flow -- clearly not the most foolproof equipment. These motors are prone to breakdown -- and due to their scappy production, malfunction had many roots to sprout from. Using the original pair of motors I was given, I noticed that, when moving forward, the robot kept veering left (and consequently had to readjust itself), which meant it moved foward in a severely erratic manner -- going forward every few centimeters, and pausing to turn back on track. After measuring with a multimeter, I found that the voltage going to both was the same. Yet the left motor was still weaker -- a sign of bad equipment. I painstakingly revised my code and 'discovered' the existence of Enable pins on the drive board (pins that indireclty control speed of motors). I used PWM control (Pulse Width Modulation, essentially reducing the average voltage going through without directly changing voltage) to weaken the right motor -- this way ensuring equality and the bare-minimum functioning of my bot. This led to the bot being too weak to carry itself normally. Yet, I had to compromise -- though it crutched and dragged along, the bot still fulfilled its purpose, and switching anything out would be too risky of a maneuver considering my inexperience with all the equipment, libraies, and modules back then. 
